  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {[{
          icon: <FiDollarSign className="text-indigo-500 mr-3" size={24} />,
          label: "Total Expenses",
          value: `$${totalExpenses.toFixed(2)}`
        },
        {
          icon: <TfiReceipt className="text-yellow-500 mr-3" size={24} />,
          label: "Pending",
          value: pendingExpenses
        },
        {
          icon: <FiCreditCard className="text-green-500 mr-3" size={24} />,
          label: "Approved",
          value: approvedExpenses
        }].map((card, idx) => (
          <motion.div key={idx} className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
      ...
      <motion.div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-x-auto">
        <table className="w-full">
          <thead className="bg-gray-50 dark:bg-gray-700">
            <tr>
              {["Date","Category","Amount","Description","Status","Actions"].map(h => (
          ...
          <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
            {expenses.map((exp) => (
              <tr key={exp.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{formatDate(exp.date)}</td>
                <td className="px-6 py-4 text-sm text-gray-900 dark:text-white">
                  {editingId === exp.id ? (
                    <input value={exp.category} onChange={(e) => handleEditChange(exp.id, "category", e.target.value)} className="border px-2 py-1"/>
                  ) : exp.category}
                </td>
                <td className="px-6 py-4 text-sm text-gray-900 dark:text-white">
                  {editingId === exp.id ? (
                    <input type="number" value={exp.amount} onChange={(e) => handleEditChange(exp.id, "amount", parseFloat(e.target.value))} className="border px-2 py-1 w-20"/>
                  ) : `$${exp.amount.toFixed(2)}`}
                </td>
                <td className="px-6 py-4 text-sm text-gray-900 dark:text-white">
                  {editingId === exp.id ? (
                    <input value={exp.description} onChange={(e) => handleEditChange(exp.id, "description", e.target.value)} className="border px-2 py-1"/>
                  ) : exp.description}
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
                  {editingId === exp.id ? (
                    <select value={exp.status} onChange={(e) => handleEditChange(exp.id, "status", e.target.value)} className={`px-2 py-1 rounded ${getStatusColor(exp.status)}`}>
                      <option value="pending">pending</option>
                      <option value="approved">approved</option>
                      <option value="rejected">rejected</option>
                    </select>
                  ) : (
                    <span className={`px-2 py-1 text-xs font-medium rounded-full ${getStatusColor(exp.status)}`}>{exp.status}</span>
                  )}
                </td>
                <td className="px-6 py-4 whitespace-nowrap flex space-x-2">
                  {editingId === exp.id ? (
                    <button onClick={() => handleSave(exp.id)} className="text-green-600 hover:text-green-800"><FaSave /></button>
                  ) : (
                    <>
                      <button onClick={() => setEditingId(exp.id)} className="text-blue-600 hover:text-blue-800"><FaEdit /></button>
                      <button onClick={() => handleDelete(exp.id)} className="text-red-600 hover:text-red-800"><FaTrash /></button>
                    </>
                  )}
                </td>
              </tr>
